@kernel function KA_assemble_kernel!(y::Y, f::F, V::TV, quadrature, domain, backend::B) where {Y,F,TV,B}
    I = @index(Global)
...
function AK_assemble_kernel!(i, y::Y, f::F, V::TV, quadrature, domain, backend::B) where {Y,F,TV,B}
    elementInfo = Bcube._get_index(domain, i)
    vₑ = Bcube.blockmap_shape_functions(V, elementInfo)
    fᵥ = Bcube.materialize(f(vₑ), elementInfo)
    values = Bcube.integrate_on_ref_element(fᵥ, elementInfo, quadrature)
...
function Bcube.__update_b!(b::AbstractVector, dofs, vals, backend::KABackends)
...
function Bcube.__assemble_linear!(b, f, V, measure::Measure, backend::KABackends)
    #KA_assemble!(b, f, nothing, V, measure, backend)
    AK_assemble!(b, f, nothing, V, measure, backend)
end

function KA_assemble!(y, f, elts, V, measure, backend::B) where {B}
    ...
    KA_assemble_kernel!(backend, WORKGROUP_SIZE)(
        y,
        f,
        V,
        quadrature,
...
function AK_assemble!(y, f, elts, V, measure, backend::B) where {B}
    quadrature = Bcube.get_quadrature(measure)
    domain = get_domain(measure)
    @assert backend == get_backend(y)
    ndrange = length(DomainIterator(domain))
    kernel_f(i) = AK_assemble_kernel!(i, y, f, V, quadrature, domain, backend)
    AK.foreachindex(kernel_f, 1:ndrange, backend; block_size=WORKGROUP_SIZE)
    return nothing
end
...
function Bcube._append_bilinear!(I, J, _X::Tuple, row, col, vals, backend::KABackends)
    offset, X = _X
...
function Bcube.assemble_bilinear!(I, J, X, f, measure::Measure, U, V, backend::KABackends)
    KA_assemble_bilinear(I, J, X, f, nothing, U, V, measure, backend)
end

function Bcube.allocate_bilinear(backend::KABackends, a, U, V, T)
...
function KA_assemble_bilinear(I, J, X, f, elts::E, U, V, measure, backend) where {E}
    quadrature = Bcube.get_quadrature(measure)
    domain = get_domain(measure)
    ...
    ndrange = length(DomainIterator(domain))
    KA_assemble_bilinear_kernel!(backend, WORKGROUP_SIZE)(
        I,
        J,
        X,
        offsets,
